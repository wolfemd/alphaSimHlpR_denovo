...
I'm in favor of keeping things in a data.frame (rectangular) shape as much as possible. 
```{r, cols.print=13}
...
schemeParams
```
ideally the scheme parameters table drives year-to-year operations with minimal additional parameter inputs

## Simulate founders
Simulate founding haplotypes, add genetic architecture and sample an initial set of founding individuals (genotypes). 
```{r}
...
Goal is to create some initial historical data to drive the first selections of parents and variety development entries.
```{r}
...
Initiate pop. improvement pipeline by generating initial selections and crossing to make progeny.
```{r}
...
Get BLUPs. In the future, would like to compare to using GETGVs.
```{r}
ini_blup<-fitModel(phenoDF = initialTP)
```
Make initial selection of clones to enter into the VDP. 
Set-up initial trials
```{r}
...
# Run breeding scheme
Each of below is an input for running breeding scheme
```{r input params}
